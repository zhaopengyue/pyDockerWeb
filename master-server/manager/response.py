...
def container_info():
    ...
    for node in container_messages:
        ...
        if container_message_message.get('statusCode') > 1:
            continue
        for container in container_message_message.get('message'):
            if container.get('statusCode') > 1:
                continue
            if container.get('message').get('paused'):
                status = 'paused'
            elif container.get('message').get('running'):
...
@app.route('/container/info_list/', methods=['POST'])
def container_info_list():
    ...
    node = container_info.get('message')
    for container_message in node:
        host = container_message.get('host')
        container_message_message = container_message.get('message')
        if container_message_message.get('statusCode') > 1:
            continue
        for container in container_message_message.get('message'):
            if container.get('statusCode') > 1:
...
@app.route('/container/operator_list/', methods=['POST'])
def container_op_list():
    """ 通过ip列表操作容器
    ...
    args = rq_args.get('args')
    op_result = _container.get_all_containers_by_list(hosts, action_type, containers, **args)
...
def image_info():
    ...
    if cluster_id not in all_cluster_id:
        _logger.write('image_info: ' + str(cluster_id) + ' is illegal or not found node in cluster')
        return jsonify({'errMessage': str(cluster_id) + '集群IP不合法或无加入节点', 'message': None, 'statusCode': 5})
    images_info = _image.get_all_images(cluster_id)
    info = []
    image_use_dict = {}
    image_use_list = set()
    ...
    for node in containers_info.get('message'):
        ...
        if container_message.get('statusCode') > 1:
            continue
        for container in container_message.get('message'):
            container_image = container.get('message').get('image')
            if ':' not in container_image:
                container_image += ':latest'
            image_use_list.add(container_image)
...
def node_download():
    ...
    if cluster_id not in all_cluster_id:
        ...
        return jsonify({'errMessage': str(cluster_id) + ' is illegal', 'statusCode': 5})
    if not image_server:
...
def get_alive_server_list():
    ...
    #     return jsonify({'message': message, 'statusCode': 0, 'errMessage': None})
    # else:
    #     return jsonify({'errMessage': 'No mirror server is available.', 'statusCode': 7, 'message': None})
